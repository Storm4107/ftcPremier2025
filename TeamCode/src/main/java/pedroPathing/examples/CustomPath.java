 ...
 */
@Autonomous(name = "CustomPath", group = "Examples")
public class CustomPath extends OpMode {
    private Follower follower;

    private final Pose startPose = new Pose(8,108, Math.toRadians(0));
    private final Pose interPose = new Pose(40, 72, Math.toRadians(0));

    private final Pose latePose = new Pose(40, 24, Math.toRadians(180));
    private final Pose endPose = new Pose(25, 72, Math.toRadians(0));

    private PathChain autoTest;
    ...
    public void loop() {
        ...
        if (follower.atParametricEnd()) {
            follower.followPath(autoTest, true);
        }
    ...
    public void init() {
        ...
        autoTest = follower.pathBuilder()
                ...
                .addPath(new BezierLine(new Point(latePose), new Point(latePose)))
                .setLinearHeadingInterpolation(latePose.getHeading(), endPose.getHeading())
                //.addPath(new BezierLine(new Point(endPose), new Point(startPose)))
                //.setLinearHeadingInterpolation(endPose.getHeading(), startPose.getHeading())
                .build();

        follower.followPath(autoTest);
